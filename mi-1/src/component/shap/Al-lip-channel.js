...
import Al_lip_channel_graph from '../Graph/Al-lip-channel';

function Al_lip_channel() {
  ...
  const [lip, setLip] = useState(8);
  const lipChange = (event) => {
    setLip(parseFloat(event.target.value));
  };
  ...
  const submitClick = () => {
    setWeightPerLenght((7850*(Math.PI*(outerRadius - 0.596*thickness) + 2*Math.PI*thickness + 2*(side2 - outerRadius - thickness) + (side1 - 2*outerRadius)+ 2*(lip - thickness))*thickness*0.000001).toFixed(3));

    setTotalWeight(((7850*(Math.PI*(outerRadius - 0.596*thickness) + 2*Math.PI*thickness + 2*(side2 - outerRadius - thickness) + (side1 - 2*outerRadius)+ 2*(lip - thickness))*thickness*0.000001)*length).toFixed(3));

    setStripWidth((Math.PI*(outerRadius - 0.596*thickness) + 2*Math.PI*(thickness - 0.596*thickness) + 2*(side2 - outerRadius - thickness) + (side1 - 2*outerRadius) + 2*(lip - thickness)).toFixed(3));

    setOutLine((Math.PI*(2*outerRadius - thickness) + 2*Math.PI*thickness + 4*(side2 - outerRadius - thickness) + 2*(side1 - 2*outerRadius) + 2*thickness+ 4*(lip - thickness)).toFixed(3))

    setArea((thickness*(2*(side2 - outerRadius - thickness)+ 2*(lip - thickness) + Math.PI*thickness*thickness + (side1 - 2*outerRadius)) + (Math.PI/2)*(Math.pow(outerRadius,2) - Math.pow(outerRadius - thickness,2))).toFixed(3))
  };
  ...
  const create3DShapes = () => {
    ...
    shape1.moveTo(thickness, side2 - lip)
    shape1.lineTo(2*thickness, side2 - lip)
    shape1.absarc(thickness,side2 - thickness,thickness, 0, Math.PI, false)
    shape1.absarc(outerRadius, outerRadius, outerRadius, Math.PI, 3*Math.PI/2, false)
    shape1.absarc(side1 - outerRadius, outerRadius, outerRadius, 3*Math.PI/2, 0, false)
    shape1.absarc(side1 - thickness, side2 - thickness, thickness, 0, Math.PI, false)
    shape1.lineTo(side1 - 2*thickness, side2 - lip)
    shape1.lineTo(side1 - thickness, side2 - lip)
    shape1.lineTo(side1 - thickness, side2 - thickness)
    shape1.absarc(side1 - outerRadius, outerRadius, outerRadius - thickness, 0, 3*Math.PI/2, true)
    shape1.absarc(outerRadius, outerRadius, outerRadius - thickness, 3*Math.PI/2, Math.PI, true)
    shape1.lineTo(thickness, side2 - thickness)
    shape1.lineTo(thickness, side2 - lip)
    shapes.push(shape1)    
  ...
  return (
    <div>
      ...
      <h1 className="heading">AL-Lip-Channel</h1>
      ...
      <div className = "container">
        <div className='box'>
          ...
          <div className="container1">
            ...
            <input className="input-field" id="lip" type="number" value={lip} onChange={lipChange} placeholder="Type something..." />
          </div>
          <div className="container1">
        ...
        <div className='box'>
          <div ref={uchannelGraphRef}><Al_lip_channel_graph side1={side1} side2={side2} lip1 = {lip} thickness1={thickness} outerRadius1={outerRadius}  sendValuey={handleComy}/></div>
        </div>
        <div className='box'>
