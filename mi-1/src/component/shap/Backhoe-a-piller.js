...
function Backhoe_a_piller() {
  ...
  const [side1, setSide1] = useState(63);
  ...
  const create3DShapes = () => {
    ...
    shape1.moveTo(radius1, thickness)
    shape1.lineTo(radius1, 0)
    shape1.absarc(side1 - outerRadius, outerRadius, outerRadius, 3*Math.PI/2, 0, false)
    shape1.absarc(side1 + outerRadius - thickness,side2 - outerRadius + thickness, outerRadius - thickness, 2*Math.PI/2, Math.PI/2, true)
    shape1.absarc(side1 + side4 - radius2,side2 + radius2, radius2, 3*Math.PI/2, 0*Math.PI/2, false)
    shape1.absarc(side1 + side4 - radius2,side2 + side3 - radius2, radius2, 0*Math.PI/2, 1*Math.PI/2, false)
    shape1.absarc(side1 + outerRadius - thickness,side2 + side3 + outerRadius - thickness, outerRadius - thickness, 3*Math.PI/2, 2*Math.PI/2, true)
    shape1.absarc(side1 - outerRadius,side6 - outerRadius, outerRadius, 0*Math.PI/2, 1*Math.PI/2, false)
    shape1.absarc(side1 - side5 + outerRadius/Math.tan(aa*angle/2),side6 - outerRadius, outerRadius, 1*Math.PI/2, 3*Math.PI/2 - aa*angle, false)
    shape1.absarc(outerRadius + l2, side7 + outerRadius - thickness, outerRadius - thickness, 5*Math.PI/2 - aa*angle, 3*Math.PI/2, true)
    shape1.absarc(outerRadius,side7 - outerRadius, outerRadius, 1*Math.PI/2, 2*Math.PI/2, false)
    shape1.lineTo(0, radius1)
    shape1.lineTo(thickness, radius1)
    shape1.absarc(outerRadius,side7 - outerRadius, outerRadius - thickness, 2*Math.PI/2, 1*Math.PI/2, true)
    shape1.absarc(outerRadius + l2, side7 + outerRadius - thickness, outerRadius, 3*Math.PI/2, 5*Math.PI/2 - aa*angle, false)
    shape1.absarc(side1 - side5 + outerRadius/Math.tan(aa*angle/2),side6 - outerRadius, outerRadius - thickness, 3*Math.PI/2 - aa*angle, 1*Math.PI/2, true)
    shape1.absarc(side1 - outerRadius,side6 - outerRadius, outerRadius - thickness, 1*Math.PI/2, 0*Math.PI/2, true)
    shape1.absarc(side1 + outerRadius - thickness,side2 + side3 + outerRadius - thickness, outerRadius, 2*Math.PI/2, 3*Math.PI/2, false)
    shape1.absarc(side1 + side4 - radius2,side2 + side3 - radius2, radius2 - thickness, 1*Math.PI/2, 0*Math.PI/2, true)
    shape1.absarc(side1 + side4 - radius2,side2 + radius2, radius2 - thickness, 0*Math.PI/2, 3*Math.PI/2, true)
    shape1.absarc(side1 + outerRadius - thickness,side2 - outerRadius + thickness, outerRadius, Math.PI/2, 2*Math.PI/2, false)
    shape1.absarc(side1 - outerRadius, outerRadius, outerRadius - thickness, 0, 3*Math.PI/2, true)
    shape1.lineTo(radius1, thickness)
    shapes.push(shape1)

    const shape2 = new THREE.Shape(); 
    shape1.moveTo(radius1, 0)
    shape1.lineTo(radius1, thickness)
    shape1.absarc(radius1, radius1, radius1 - thickness, 3*Math.PI/2, Math.PI, true)
    shape1.lineTo(0, radius1)
    shape1.absarc(radius1, radius1, radius1, Math.PI, 3*Math.PI/2, false)
    shape1.lineTo(radius1, 0)
    shapes.push(shape2)
  ...
  useEffect(() => {
    ...
    create3DShapes();
  }, [side1,side2 ,side3,side4, side5, side6, radius1, radius2, angle1, outerRadius, thickness, length]);
  ...
  const handleDownload = () => {
    ...
    doc.setFontSize(10).setTextColor('black').text(`side(A): ${side1}   side(B): ${side2}   side(C): ${side3}   side(D): ${side4}   side(E): ${side5}   side(F): ${side6}   Radius(R1): ${radius1}   Radius(R2): ${radius2}   Angle(D): ${angle1}Thickness(t): ${thickness}   Length(L): ${length}`, 6, 30);
  ...
  return (
    <div>
      ...
      <div className = "container">
        ...
        <div className='box'>
        <div ref={tShapGraphRef}><Backhoe_a_piller_graph side11 = {side1} side22={side2} side33={side3} side44={side4} side55={side5} side66={side6} side77={side7} angle1={angle1} radius11={radius1} radius22={radius2} thickness1={thickness} outerRadius1={outerRadius} sendValuey={handleComy}/></div>
        </div>
        <div className='box'>
...
export default Backhoe_a_piller;
