...
function Beam_window_frame_1() {
  ...
  const submitClick = () => {
    setWeightPerLength((7850*(2*Math.PI*(outerRadius - 0.596*thickness) + Math.PI*(radius - 0.596*thickness) + (side1 - 2*outerRadius) + (side3 - 2*outerRadius) + (side2 - 2*outerRadius) + (side3 - side2 - radius - outerRadius + thickness) + (side1 - 2*radius - 2*outerRadius + thickness))*thickness*0.000001).toFixed(3));
  ...
  return (
    <div>
      ...
      <div className = "container">
        ...
        <div className='box'>
          <div ref={GraphRef}><Beam_window_frame_1_graph side11={side1} side22={side2} side33={side3} side44 = {side4} radius1={radius} thickness1={thickness} outerRadius1={outerRadius} sendValue={handleData}/>
        </div></div>
        <div className='box'>
