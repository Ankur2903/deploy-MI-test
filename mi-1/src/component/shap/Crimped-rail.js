...
import Crimped_rail_graph from '../Graph/Crimped-rail';

function Crimped_rail() {
  const [length, setLength] = useState(1);
  const [thickness, setThickness] = useState(0.7);
  const [side1, setSide1] = useState(10);
  const [side2, setSide2] = useState(10);
  const [side3, setSide3] = useState(7.5)
  const [side4, setSide4] = useState(7.9)
  const [outerRadius, setOuterRadius] = useState(1.05);
  ...
  const create3DShapes = () => {
    ...
    shape1.moveTo(side1 - thickness - side3, side4 - 2*thickness + 2*outerRadius) 
    shape1.lineTo(side1 - thickness - side3, side4 - thickness + 2*outerRadius)
    shape1.absarc(side1 - thickness - outerRadius,  side4 - thickness + outerRadius, outerRadius, Math.PI/2, 3*Math.PI/2, true)
    shape1.absarc(outerRadius, side4 - outerRadius, outerRadius - thickness, Math.PI/2, Math.PI, false)
    ...
    shape1.absarc(outerRadius, side2 - outerRadius, outerRadius - thickness, Math.PI/2, 3*Math.PI/2, false)
    shape1.lineTo(side3, side2 - 2*outerRadius + thickness)
    shape1.lineTo(side3, side2 - 2*outerRadius)
    shape1.absarc(outerRadius, side2 - outerRadius, outerRadius, 3*Math.PI/2, Math.PI/2, true)
    ...
    shape1.absarc(outerRadius, side4 - outerRadius, outerRadius, Math.PI, Math.PI/2, true)
    shape1.absarc(side1 - thickness - outerRadius,  side4 - thickness + outerRadius, outerRadius - thickness, 3*Math.PI/2, Math.PI/2, false)
    shape1.lineTo(side1 - thickness - side3, side4 - 2*thickness + 2*outerRadius) 
    shapes.push(shape1)    
  ...
  return (
    <div>
      ...
      <h1 className="heading">Crimped Rail</h1>
      ...
      <div className = "container">
        ...
        <div className='box'>
          <div ref={lAngleGraphRef}><Crimped_rail_graph side11={side1} thickness1={thickness} side22={side2} side33={side3} side44={side4} outerRadius1={outerRadius} sendValuex={handleComx} sendValuey={handleComy}/></div>
          </div>
        <div className='box'>
