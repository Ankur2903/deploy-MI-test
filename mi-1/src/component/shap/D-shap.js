...
function D_shap() {
  ...
  const submitClick = () => {
    setWeightPerLength((((side1 -2*outerRadius) + (side2 - 2*outerRadius) + (side1-side2) + 3*(Math.PI*(outerRadius - 0.596*thickness))/2 + (Math.PI*(side2-outerRadius - thickness/2))/2)*7850*thickness*0.000001).toFixed(3));
  ...
  return (
    <div>
      ...
      <div className = "container">
        ...
        <div className='box'>
        <div ref={GraphRef}><D_shap_graph side11={side1} thickness1={thickness} side22={side2} outerRadius1={outerRadius} sendValue={handleData}/>
        </div></div>
        <div className='box'>
