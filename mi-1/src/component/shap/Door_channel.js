...
import Door_channel_graph from '../Graph/Door-channel';
import '../../App.css'
import 'jspdf-autotable';
import Result from './Result';

function Door_channel() {
  ...
  const [side1, setSide1] = useState(20);
  ...
  const submitClick = () => {
    ...
    setTotalWeight((7850*((side1-2*outerRadius) + (side2 - 2*outerRadius) + (side3-2*outerRadius + thickness) + (side1-4*outerRadius+thickness) +  2*(lip - outerRadius) + 3*Math.PI*(outerRadius - 0.596*thickness))*thickness*0.000001*length).toFixed(2));

    setStripWidth(((side1-2*outerRadius) + (side2 - 2*outerRadius) + (side3-2*outerRadius + thickness) + (side1-4*outerRadius+thickness) +  2*(lip - outerRadius) + 3*Math.PI*(outerRadius - 0.596*thickness)).toFixed(3))
  ...
  const doorChannelGraphRef = useRef()

  const handleDownload = () => {
    const doc = new jsPDF();
    html2canvas(doorChannelGraphRef.current).then((canvas) => {
    ...
    doc.setFontSize(10).setTextColor('black').text(`Side(A): ${side1}   Side(B): ${side2}   Side(C): ${side3}   Side(D): ${side4}   Lip(l): ${lip}   Thickness(t): ${thickness}   Length(L): ${length}`, 6, 30);
  ...
  const create3DShapes = () => {
    ...
    shape1.moveTo(lip,side1 - thickness);
    shape1.moveTo(lip,side1);
    ...
    shape1.lineTo(side2 - outerRadius,0)   
    shape1.absarc(side2 - outerRadius,outerRadius,outerRadius,3*Math.PI/2,0*Math.PI/2,false); 
    shape1.lineTo(side2,side1 - side4 - outerRadius + thickness)
    ...
    shape1.lineTo(side2 + side3 - outerRadius,side1 - side4)
    shape1.absarc(side2 + side3 - outerRadius,side1 - side4 + outerRadius,outerRadius,3*Math.PI/2,0*Math.PI/2,false); 
    shape1.lineTo(side2 + side3,side1 - outerRadius)
    shape1.absarc(side2 + side3 - outerRadius,side1 - outerRadius,outerRadius,0*Math.PI/2,1*Math.PI/2,false); 
    shape1.lineTo(side2 + side3 - lip, side1);
    shape1.lineTo(side2 + side3 - lip, side1 - thickness);
    shape1.lineTo(side2 + side3 - outerRadius, side1 - thickness);
    shape1.absarc(side2 + side3 - outerRadius,side1 - outerRadius,outerRadius - thickness,1*Math.PI/2,0*Math.PI/2,true); 
    shape1.lineTo(side2 + side3 - thickness,side1 - side4 + outerRadius)
    shape1.absarc(side2 + side3 - outerRadius,side1 - side4 + outerRadius,outerRadius - thickness,0*Math.PI/2,3*Math.PI/2,true); 
    shape1.lineTo(side2 + outerRadius - thickness, side1 - side4 + thickness)
    shape1.absarc(side2 + outerRadius - thickness,side1 - side4 - outerRadius + thickness,outerRadius,1*Math.PI/2,2*Math.PI/2,false); 
    shape1.lineTo(side2 - thickness,outerRadius)
    ...
    shape1.lineTo(lip,side1 - thickness)
    shapes.push(shape1)    
     ...
     <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative'}}>
      ...
      <div className = "container">
        ...
        <div style={{ fontWeight: 'bold' }}>Input</div>
          ...
          <div className="container1">
            <lable className="label" htmlFor="side4">Lip (l) mm</lable>
