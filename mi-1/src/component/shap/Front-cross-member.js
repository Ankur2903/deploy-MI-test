...
function Front_cross_member() {
  ...
  const submitClick = () => {
    setWeightPerLenght((7850*((4*Math.PI - 2*aa*angle)*(outerRadius - 0.596*thickness) + l1 + (x3 - x4) + (side1 + side3 - 2*outerRadius*(Math.tan(aa*angle/2) + 1/Math.tan(aa*angle/2))) + (side2 + side4)/Math.sin(aa*angle))*thickness*0.000001).toFixed(3));
  ...
  return (
    <div>
      ...
      <div className = "container">
        ...
        <div className='box'>
        <div ref={tShapGraphRef}><Front_cross_member_graph side11 = {side1} side22={side2} side33={side3} side44={side4} angle={angle} thickness1={thickness} outerRadius1={outerRadius} sendValuey={handleComy}/></div>
        </div>
        <div className='box'>
