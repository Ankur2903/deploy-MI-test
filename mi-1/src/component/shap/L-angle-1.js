...
import '../../App.css'
import L_angle_1_graph from '../Graph/L-angle-1';
...
function L_angle_1() {
  ...
  const submitClick = () => {
    setWeightPerLength(((3*Math.PI*(outerRadius - 0.596*thickness)/2+ (side2 - 2*outerRadius) + (side3 - outerRadius) + (side4 - outerRadius) +  (side1 - 2*outerRadius))*thickness*7850*0.000001).toFixed(3));

    setTotalWeight(((3*Math.PI*(outerRadius - 0.596*thickness)/2+ (side2 - 2*outerRadius) + (side3 - outerRadius) + (side4 - outerRadius) +  (side1 - 2*outerRadius))*thickness*7850*0.000001 * length).toFixed(2));

    setStripWidth((3*Math.PI*(outerRadius - 0.596*thickness)/2 + (side2 - 2*outerRadius) + (side3 - outerRadius) + (side4 - outerRadius) +  (side1 - 2*outerRadius)).toFixed(3))

    setOutLine((3*Math.PI*(2*outerRadius - thickness)/2 + 2*(side2 - 2*outerRadius) +  2*(side1 - 2*outerRadius) + 2*(side3 - outerRadius) +  2*(side4 - outerRadius) + 2* thickness).toFixed(3));

    setArea((thickness*((side2 - 2*outerRadius) +  (side1 - 2*outerRadius) + (side3 - outerRadius) +  (side4 - outerRadius)) + 3*(Math.PI/4)*(Math.pow(outerRadius,2) - Math.pow(outerRadius - thickness,2))).toFixed(3))
  ...
  return (
    <div>
      ...
      <h1 className="heading">U-Channel Profile</h1>
      ...
      <div className = "container">
        ...
        <div className='box'>
          <div ref={GraphRef}><L_angle_1_graph side11={side1} thickness1={thickness} side22={side2} side33={side3} side44={side4} outerRadius1={outerRadius} sendValue={handleData}/></div>
        </div>
        <div className='box'>
