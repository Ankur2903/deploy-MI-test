...
import Niko_bts_graph from '../Graph/Niko bts';
...
function Niko_bts() {
  ...
  const [angle, setAngle] = useState(164);
  ...
  const y1 = side2 - outerRadius/Math.tan(aa*angle/2)

  const x2 = x1 + l*Math.sin(aa*angle)
  const y2 = y1 - l*Math.cos(aa*angle)

  const x3 = side1 - outerRadius
  const y3 = side2 - outerRadius/Math.tan(aa*angle/2)

  const x4 = x3 - l*Math.sin(aa*angle)
  const y4 = y3 - l*Math.cos(aa*angle)

  const submitClick = () => {
    setWeightPerLenght((7850*(2*Math.PI*(outerRadius - 0.596*thickness) + 2*l + 2*(side1 - 2*outerRadius) + 2*(side2 - outerRadius - outerRadius/Math.tan(aa*angle/2)) + 2*(side4 - outerRadius/Math.tan(aa*angle1)))*thickness*0.000001).toFixed(3));
  ...
  const create3DShapes = () => {
    ...
    shape1.moveTo(x2 - outerRadius/Math.tan(aa*angle1/2) + side4, side3 + side2 - thickness)
    shape1.lineTo(x2 - outerRadius/Math.tan(aa*angle1/2) + side4, side3 + side2)
    shape1.absarc(x2,y2, outerRadius, Math.PI/2, 3*Math.PI/2 - aa*angle1, false)
    shape1.absarc(x1,y1, outerRadius, 3*Math.PI/2 - aa*angle1, Math.PI, false)
    ...
    shape1.absarc(x3, y3, outerRadius, 0, Math.PI - aa*angle, false)
    shape1.absarc(x4, y4, outerRadius, Math.PI - aa*angle, Math.PI/2, false)
    shape1.lineTo(x4 + outerRadius/Math.tan(aa*angle1/2) - side4, side2 + side3)
    shape1.lineTo(x4 + outerRadius/Math.tan(aa*angle1/2) - side4, side2 + side3 - thickness)
    shape1.absarc(x4, y4, outerRadius - thickness, Math.PI/2, Math.PI - aa*angle, true)
    shape1.absarc(x3, y3, outerRadius - thickness, Math.PI - aa*angle, 0, true)
    ...
    shape1.absarc(x1,y1, outerRadius - thickness, Math.PI, 3*Math.PI/2 - aa*angle1, true)
    shape1.absarc(x2,y2, outerRadius - thickness, 3*Math.PI/2 - aa*angle1, Math.PI/2, true)
    shape1.lineTo(x2 - outerRadius/Math.tan(aa*angle1/2) + side4, side3 + side2 - thickness)
    shapes.push(shape1)
  ...
  return (
    <div>
      ...
      <h1 className="heading">Niko BTS</h1>
      ...
      <div className = "container">
        ...
        <div className='box'>
        <div ref={tShapGraphRef}><Niko_bts_graph side11 = {side1} side22={side2} side33={side3} side44={side4} angle={angle} thickness1={thickness} outerRadius1={outerRadius} sendValuey={handleComy}/></div>
        </div>
        <div className='box'>
