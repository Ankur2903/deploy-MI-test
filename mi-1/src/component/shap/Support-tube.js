...
function Support_tube() {
  ...
  const submitClick = () => {
    setWeightPerLength((7850*(2*Math.PI*(outerRadius - 0.596*thickness) + 2*l + 2*(side3 - outerRadius) + 2*(side1 - 2*outerRadius - l*Math.cos(angle*aa/2)))*thickness*0.000001).toFixed(3));

    setTotalWeight((7850*(2*Math.PI*(outerRadius - 0.596*thickness) + 2*l + 2*(side3 - outerRadius) + 2*(side1 - 2*outerRadius - l*Math.cos(angle*aa/2)))*thickness*0.000001*length).toFixed(3));

    setStripWidth((2*Math.PI*(outerRadius - 0.596*thickness) + 2*l + 2*(side3 - outerRadius) + 2*(side1 - 2*outerRadius - l*Math.cos(angle*aa/2))).toFixed(3));

    setOutLine((2*Math.PI*(2*outerRadius - thickness) + 4*l + 4*(side3 - outerRadius) + 4*(side1 - 2*outerRadius - l*Math.cos(angle*aa/2)) + 2*thickness).toFixed(3))

    setArea((Math.PI*(Math.pow(outerRadius,2) - Math.pow(thickness,2)) + thickness*(2*l + 2*(side3 - outerRadius) + 2*(side1 - 2*outerRadius - l*Math.cos(angle*aa/2)))).toFixed(3))
  ...
  const create3DShapes = () => {
    ...
    shape1.moveTo(side3, thickness)
    shape1.lineTo(side3, 0)
    shape1.absarc(outerRadius, outerRadius, outerRadius, 3*Math.PI/2, Math.PI, true)
    shape1.absarc(outerRadius, side1 - l*Math.cos(angle*aa/2) - outerRadius, outerRadius, Math.PI, Math.PI - aa*angle/2, true)
    shape1.absarc(side2/2, side1 - outerRadius, outerRadius, Math.PI - aa*angle/2, aa*angle/2, true)
    shape1.absarc(side2 - outerRadius, side1 - l*Math.cos(angle*aa/2) - outerRadius, outerRadius, aa*angle/2, 0, true)
    shape1.absarc(side2 - outerRadius, outerRadius, outerRadius, 0, 3*Math.PI/2, true)
    shape1.lineTo(side2 - side3, 0)
    shape1.lineTo(side2 - side3, thickness)
    shape1.absarc(side2 - outerRadius, outerRadius, outerRadius - thickness, 3*Math.PI/2, 0, false)
    shape1.absarc(side2 - outerRadius, side1 - l*Math.cos(angle*aa/2) - outerRadius, outerRadius - thickness, 0, aa*angle/22, false)
    shape1.absarc(side2/2, side1 - outerRadius, outerRadius - thickness, aa*angle/2, Math.PI - aa*angle/2, false)
  ...
  const handleDownload = () => {
    ...
    doc.setFontSize(10).setTextColor('black').text(`side(A): ${side1}   side(B): ${side2}   side(C): ${side3} Angle(D): ${angle}   Thickness(t): ${thickness}   Length(L): ${length}`, 6, 30);
  ...
  return (
    <div>
      ...
      <h1 className="heading">Support Tube</h1>
      ...
      <div className = "container">
        ...
        <div className='box'>
        <div ref={GraphRef}><Support_tube_graph side11 = {side1} side22={side2} side33={side3} angle={angle} thickness1={thickness} outerRadius1={outerRadius} sendValue={handleData}/></div>
        </div>
        <div className='box'>
