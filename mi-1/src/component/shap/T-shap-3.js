...
function T_shap_3() {
  ...
  const submitClick = () => {
    setWeightPerLength((7850*(2*Math.PI*(outerRadius - 0.596*thickness) + 2*Math.PI*(thickness - 0.596*thickness) + (side2 - 2*outerRadius) + (side3 - 2*outerRadius) + 2*x + 2*l + 2*(side2 - outerRadius - outerRadius*Math.tan(Math.PI/4 - aa*angle/2)))*thickness*0.000001).toFixed(3));
  ...
  return (
    <div>
      ...
      <div className = "container">
        ...
        <div className='box'>
        <div ref={GraphRef}><T_shap_graph_3 side11 = {side1} side22={side2} side33={side3} angle1={angle} thickness1={thickness} outerRadius1={outerRadius} sendValue={handleData}/></div>
        </div>
        <div className='box'>
